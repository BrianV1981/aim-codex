...
### 10.1 The Mandatory Rule
Every GitHub repository managed by A.I.M. **MUST** utilize a Semantic Release Pipeline. 

### 10.2 Conventional Commits
To drive this automation, all commit messages executed by the agent must follow the Conventional Commits specification. The `aim push` command explicitly relies on these prefixes to calculate version numbers and generate changelogs:
...
### 10.3 Automated Changelog
The public face of A.I.M.'s memory is the `CHANGELOG.md`. This file is not written by hand or generated by a summarizer LLM. It is strictly assembled by the Semantic Release engine reading the commit history during deployment, ensuring absolute fidelity between the code shipped and the history recorded.
